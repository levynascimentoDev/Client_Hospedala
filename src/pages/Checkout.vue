...
export default defineComponent({
    ...
    methods: {
        ...
        async refreshCode() {
            this.loading = true
            const resp = await fetch(`https://api.hospedala.com/api/auth/token/refresh`, {
                method:"PATCH",
                headers:{
    ...
    async created() {
        ...
        if (this.router.params.code && this.router.params.type && ["register", "forgot-password", "login"].includes(this.router.params.type as string)) {            
            if (this.router.params.type == "login") {
                ...
                const resp = await fetch(`https://api.hospedala.com/api/auth/login/tokens`, {
                    headers:{
                        "Content-Type":"application/json",
    ...
    watch:{
        async fullCode(after:string, _:string) {
            if (after.length == 5 && !isNaN(Number(after))) {
                ...
                if (this.router.params.type == "login") {
                    const resp = await fetch(`https://api.hospedala.com/api/auth/login/verify`, {
                        method:"POST",
                        headers:{
                            "Content-Type":"application/json"
                        },
                    ...
                    const resp = await fetch(`https://api.hospedala.com/api/auth/register/verify`, {
                        method:"POST",
                        headers:{
                            "Content-Type":"application/json"
                        },
